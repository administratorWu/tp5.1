...
class XgRedis extends Model
{
    ...
    public function get_redis_page_info($hash_prefix, $page, $pageSize, $key = array())
    {
        ...
        $range = $redis->zrange($hash_prefix . '_sort', $limit_s, $limit_e); //指定区间内，带有 score 值(可选)的有序集成员的列表。 
        $count = $redis->zcard($hash_prefix . '_sort'); //统计ScoreSet总数
        $pageCount = ceil($count / $pageSize); //总共多少页
        $pageList = array();
        foreach ($range as $qid) {
            if (count($key) > 0) {
                $pageList[] = $redis->hmget($hash_prefix . '_' . $qid, $key); //获取hash表中所有的数据
            } else {
    ...
    {
        $redis = new  Redis();
        $info = $redis->hgetall($hash_prefix . '_' . $id);
        return $info;
    }
    ...
    public function del_redis_page_info($hash_prefix, $ids)
    {
        ...
        foreach ($ids as $value) {
            ...
            $redis->del($hashName);
            $redis->zrem($hash_prefix . '_sort', $value);
        }
        return true;
    }
    ...
    //模糊删除
    public function vague_del($name)
    {
        $redis = new  Redis();
        $postlist = $redis->keys($name . '*');
